...
app = FastAPI(title='Wood AI CML ALO API', version='0.3.0', description='ML API for CML optimization')
...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'risk_score' not in df.columns:
        df['risk_score'] = (df['average_corrosion_rate'] * 20 + (10 - df['thickness_mm']) * 5).clip(0, 100)
    return df

@app.get('/')
async def root():
    return {'message': 'Wood AI CML Optimization API', 'version': '0.3.0', 'documentation': '/docs'}

@app.get('/health', response_model=HealthResponse)
async def health():
    return {'status': 'ok', 'model_loaded': model is not None, 'model_path': str(MODEL_PATH) if MODEL_PATH.exists() else None, 'version': '0.3.0'}
...
async def upload_cml_data(file: UploadFile = File(...)):
    try:
        ...
        return {'filename': file.filename, 'rows': len(df), 'columns': list(df.columns), 'preview': df.head(5).to_dict('records'), 'message': f'Successfully uploaded {len(df)} records', 'validation': result}
    except Exception as e:
...
async def score_cml_data(file: UploadFile = File(...)):
    ...
    try:
        ...
        feature_cols = ['average_corrosion_rate', 'thickness_mm', 'commodity', 'feature_type', 'cml_shape', 'remaining_life_years', 'corrosion_thickness_ratio', 'risk_score', 'days_since_inspection']
        ...
        for idx, row in df.iterrows():
            results.append({'id_number': str(row['id_number']), 'predicted_elimination_flag': int(predictions[idx]), 'elimination_probability': float(probabilities[idx]), 'recommendation': 'ELIMINATE' if predictions[idx] == 1 else 'KEEP', 'confidence': 'HIGH' if abs(probabilities[idx] - 0.5) > 0.3 else 'MODERATE'})
        logger.info(f'Successfully scored {len(df)} CMLs')
        return {'rows_scored': len(df), 'results': results[:100], 'total_results': len(results), 'model_info': {'model_type': type(model).__name__, 'features_used': feature_cols}, 'message': f'Successfully scored {len(df)} CML records'}
    except Exception as e:
...
async def forecast_remaining_life(file: UploadFile = File(...)):
    try:
        ...
        if not validation['valid']:
             logger.warning(f"Validation failed for {file.filename}: {validation['errors']}")
        ...
        for _, row in df.iterrows():
            try:
                ...
                results.append(ForecastOutput(
                    ...
                    estimated_thickness_at_next_inspection=schedule['estimated_thickness_at_next_inspection'],
